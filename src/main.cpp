...
#if defined(NDEBUG)
# error "Surajcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xa0f4bf3ffd2d8da8f4c6e7efa9c3eb75a1806d80bf7491712ebb057456d73188");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Surajcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Surajcoin Signed Message:\n";
...
{
    // Surajcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Surajcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 30; // Surajcoin: 5 minutes
static const int64 nTargetSpacing = 1 * 30; // Surajcoin: 30 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Surajcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Surajcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xa0f4bf3ffd2d8da8f4c6e7efa9c3eb75a1806d80bf7491712ebb057456d73188");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "This is Suraj New Test Coin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0x87e2f770a9362cfc8811bef91693478565e521e27b31b28ca51c4cca3ecbcf04"));
        if (true && block.GetHash() != hashGenesisBlock)
	{
		printf("Searching for genesis block...\n");
		//this will figure out a valid hash and Nonce if you'readable
		//crating a different genesis block
		...
		loop
		{
			...
			{
				printf("nonce %o8X: hash = %s (target = %s)\n", block.nNonce, thash.ToString().c_str(), hashTarget.ToString().c_str()); 
			}	
			++block.nNonce;
			if(block.nNonce == 0)
			{
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Surajcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SurajcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SurajcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SurajcoinMiner(CWallet *pwallet)
{
    printf("SurajcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("surajcoin-miner");
    ...
    try { loop {
        ...
        printf("Running SurajcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("SurajcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SurajcoinMiner, pwallet));
}
